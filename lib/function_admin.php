...
function sendNewsletterToSubscribers($newsletter_id, $title, $template_with_placeholders) {
    ...
    try {
        logMessage("Začínám odesílání newsletteru ID: $newsletter_id");
        
        $stmt = $pdo->prepare("UPDATE newsletters SET status = 'sending' WHERE id = ?");
        ...
        if (empty($subscribers)) {
            ...
            $stmt = $pdo->prepare("UPDATE newsletters SET status = 'sent', sent_at = NOW(), sent_count = 0 WHERE id = ?");
            $stmt->execute([$newsletter_id]);
        ...
        foreach ($subscribers as $email) {
            try {
                ...
                if (sendSimpleEmail($email, $title, $personalized_html)) {
                    $sent_count++;
                    
                    if ($pdo->query("SHOW TABLES LIKE 'newsletter_sent_log'")->rowCount() > 0) {
                        $stmt = $pdo->prepare("INSERT INTO newsletter_sent_log (newsletter_id, email, status, sent_at) VALUES (?, ?, 'sent', NOW())");
                        $stmt->execute([$newsletter_id, $email]);
                    }
                } else {
                    $failed_count++;
                    
                    if ($pdo->query("SHOW TABLES LIKE 'newsletter_sent_log'")->rowCount() > 0) {
                        $stmt = $pdo->prepare("INSERT INTO newsletter_sent_log (newsletter_id, email, status, error_message, sent_at) VALUES (?, ?, 'failed', 'Mail function returned false', NOW())");
                        $stmt->execute([$newsletter_id, $email]);
                    }
                }
                ...
                $failed_count++;
                logMessage("Chyba při odesílání na $email: " . $e->getMessage());
                
                if ($pdo->query("SHOW TABLES LIKE 'newsletter_sent_log'")->rowCount() > 0) {
                    $stmt = $pdo->prepare("INSERT INTO newsletter_sent_log (newsletter_id, email, status, error_message, sent_at) VALUES (?, ?, 'failed', ?, NOW())");
                    $stmt->execute([$newsletter_id, $email, $e->getMessage()]);
                }
            }
        }

        $stmt = $pdo->prepare("UPDATE newsletters SET status = 'sent', sent_at = NOW(), sent_count = ? WHERE id = ?");
        $stmt->execute([$sent_count, $newsletter_id]);

        logMessage("Newsletter ID: $newsletter_id odeslán. Úspěšně: $sent_count, Neúspěšně: $failed_count");
        
        return $sent_count;
    } catch (Exception $e) {
        logMessage("CHYBA při odesílání newsletteru ID $newsletter_id: " . $e->getMessage());
        
        $stmt = $pdo->prepare("UPDATE newsletters SET status = 'failed' WHERE id = ?");
        $stmt->execute([$newsletter_id]);
